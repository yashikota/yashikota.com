...
最近はCloudflare Workers推しで、無料でグローバルエッジ環境に即時デプロイ、フロントとバックを一緒くたに扱えアプリをぱぱっと作れるので好きです😍  

あとは自宅サーバーでk8s盆栽を始めたり、Observabilityに手を出してみたりして色々遊んでいます。  
CI/CDとかも4年ほど、主に個人開発/インターンで使用していて、GitHub Actionsをメインで使用しており、Lint/Format/Test/Deploy/Releaseを回すのはもちろん、スクレイピングとかにも使ってます。ボタンポチでどこからでも実行できるので楽で好きです。  
...
最近はLLMが進歩してきて仕様書からコードに落とし込むところはかなりできていると思うんですが、やっぱり人間社会で営まれる活動とコンピュータの住むデジタルな信号処理の世界を結びつけるのは人じゃないとまだまだ無理だなと感じているので、それも含めて大域的に物事を見るテックリード的な立場になるのが面白そうかなとは思っています。  

Pythonは最初は何でもこれで書いていたんですが、型とか環境構築とかうーんとなって若干離れ気味になったんですけど、最近もLLMを使ったアプリケーションとかを開発するとなるとPythonが必須なので逃れられないです。でもuvとかruffみたいなツールが出てきてだいぶ不満は無くなってきたので開発元には大感謝🙏  

Goはクロスプラットフォームにシングルバイナリで実行ファイルをビルドできるところが好きで、ちょっとしたCLIアプリとかは基本Goで書いています。  
